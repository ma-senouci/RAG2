# RAG Custom Orchestration — Production-Ready RAG System

[![Live Demo](https://img.shields.io/badge/🤗_Live_Demo-Try_It-green?style=for-the-badge)](https://huggingface.co/spaces/m-senouci/rag-custom-orchestration)
[![CI](https://github.com/ma-senouci/rag-custom-orchestration/actions/workflows/ci.yml/badge.svg)](https://github.com/ma-senouci/rag-custom-orchestration/actions/workflows/ci.yml)
...
```bash
git clone https://github.com/ma-senouci/rag-custom-orchestration.git
cd rag-custom-orchestration
...
This project uses a **hand-built streaming pipeline** — no framework abstractions or black-box handlers:
...
### 1. Build the Image
```bash
docker build -t rag-custom-orchestration .
```
...
docker run -p 7860:7860 -e DEEPSEEK_API_KEY=your-api-key-here rag-custom-orchestration

# Switch provider and override default model
docker run -p 7860:7860 -e LLM_BACKEND=openai -e OPENAI_MODEL=gpt-4.1 -e OPENAI_API_KEY=your-key rag-custom-orchestration
```
...
> ```bash
> docker run -p 7860:7860 -e DEEPSEEK_API_KEY=xxx -v /path/to/docs:/app/docs rag-custom-orchestration sh -c "python app.py --sync && python app.py"
> ```

## 📁 Project Structure

```
rag-custom-orchestration/
...
> **🚀 [Try the live demo on HuggingFace Spaces →](https://huggingface.co/spaces/m-senouci/rag-custom-orchestration)**
